...
namespace ANCIA.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public async Task<IActionResult> MakeUser([FromBody] UpdatePermissionDto updatePermissionDto)
        {
            ...
            if (response.Status)
            {
                    var message = new Message(new string[] { response.Email },"Congratulation","you have Guaranted for <b>User Role</b> As Requested");
        ...
        public async Task<IActionResult> MakeUserAdmin([FromBody] UpdatePermissionDto updatePermissionDto)
        {
            ...
            if (response.Status)
            {
                var message = new Message(new string[] { response.Email }, "Congratulation", "you have Guaranted for <b>Admin Role</b> As Requested");
                _emailService.SendEmail(message);
        ...
        public async Task<IActionResult> MakeUserOwner(UpdatePermissionDto updatePermissionDto)
        {
            ...
            if (response.Status)
            {
                var message = new Message(new string[] { response.Email }, "Congratulation ", "you have Guaranted for <b>Owner Role</b> As Requested");
                _emailService.SendEmail(message);
                return Ok(new Response { Status = response.Status, Message = response.Message, StatusCode = response.StatusCode });
            }
            return Ok(new Response { Status = response.Status, Message = response.Message, StatusCode = response.StatusCode });
        ...
        public async Task<ActionResult<Response>> Create([FromBody] RegisterUser user)
        {
           var response = await _userService.CreateuserAsync(user);
            if (response.Status == true)
            {
                ...
                var message = new Message(new string[] { response.Email }, " Password And Conformation Link", " <h4> Well Come to Our Org, your Password is :<H2> " + response.Password + ".</H2> <br></br>!!! Please Comfirm with this Link <b>Unless you are not conformed./b> </H4>" + conformationLink);
                _emailService.SendEmail(message);
            }
            return Ok(new Response { Status = response.Status,Message = response.Message,StatusCode = response.StatusCode});
        ...
        var response = await _userService.LoginAsync(login);
            if (response.Status == true)
            {
                var message = new Message(new string[] { response.Email }, "OTP from Sola Please Confierm it ASAP", response.Token);
                _emailService.SendEmail(message);
            }
                return Ok(new Response { Status = response.Status, Message = response.Message,StatusCode = response.StatusCode });
        }
        ...
        public async Task<IActionResult> ForgetPassword([Required] string email)
        {
            ...
            var message = new Message(new string[] { response.Email }, "Forget Password Email", forgotePasswordlink!);
            _emailService.SendEmail(message); 
        ...
        {
            var message = new Message(new string[] { "solomonsefiw91@gmail.com","solayemam1234@gmail.com" }, "Melti Msg Test Message", "this is test message");
            _emailService.SendEmail(message);
